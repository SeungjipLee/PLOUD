...
const CreateBoard = () => {
  ...
  const handlePut = async (e) => {
    ...
    if (formData.title === "" || formData.content === "" || postVideoId == 0) {
      await setMessage("제목과 내용과 영상을 모두 입력해주세요.");
      setAlert(true);
    } else {
      putboard(
        token,
    ...
    <>
      <div className="bg-white w-full min-h-screen">
        <Page header={<Navbar />} footer={<Footer />}>
                ...
                <div className="flex">
                  ...
                  {isSelectedVideo && (
                    <div className="pt-0.5">
                      선택된 동영상 : {selectedVideoTitle}
                    </div>
                  )}
                </div>
                <Modal
                  className="speechListModal"
                  isOpen={isModalOpen}
                ...
                >
                  ...
                  <ul>
                    {userVideoList.map((video, index) => (
                      <li
                        key={index}
                        className="grid grid-col-12 text-center px-5 py-2 mb-3 modalList"
                        onClick={() => handleVideoSelect(video)}
                      >
                        <div className="text-xl font-bold">{video.title}</div>
                        <div className="col-2 mx-2 pt-0.5">
                          {video.playTime}
                        </div>
                        <div className="col-6 pt-1 text-sm ">
                          {formatDate(video.recordTime)}
                        </div>
                      </li>
                    ))}
                  </ul>
          ...
          <div
            className="createBtn px-60 mt-6 mb-24"
            style={{ justifyContent: "center" }}
          >
            {isCreate && (
              <button
                onClick={handleSubmit}
                className="border writeBtn rounded-md py-1 px-4 mx-10"
              >
                등록
              </button>
            )}
            ...
            <button
              className="border writeBtn1 rounded-md py-1 px-4 mx-10"
