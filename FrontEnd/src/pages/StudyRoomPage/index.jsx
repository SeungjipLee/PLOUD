...
const StudyRoomPage = () => {
  ...
  const joinSession = () => {
    ...
    session.current.on("streamCreated", (event) => {
      ...
      let username = JSON.parse(event.stream.connection.data).clientData;
      setChatList([...chatList, {username : username, content : "님이 입장하였습니다."}]);
    ...
    session.current.on("streamDestroyed", (event) => {
      ...
      console.log(JSON.parse(event.stream.connection.data).clientData);
      let username = JSON.parse(event.stream.connection.data).clientData;
      setChatList([...chatList, {username : username, content : "님이 퇴장하였습니다."}]);
    ...
    session.current.on("signal:chat", (event) => {
      ...
      setChatList([...chatList, {username: "" ,content: JSON.parse(event.data).chatvalue}]);
    });
