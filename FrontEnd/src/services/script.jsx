...
export const getCategoryList = async (token, success, fail) => {
  return await API(token).get("script/list").then(success).catch(fail);
};
...
export const getScriptDetail = async (token, scriptId, success, fail) => {
  return await API(token).get(`script/${scriptId}`).then(success).catch(fail);
};
...
export const registScript = async (token, data, success, fail) => {
  return await API(token).post("script/regist", data).then(success).catch(fail);
};
